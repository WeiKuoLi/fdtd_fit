...
import math
from test import create_test_movie
...
class CosineModel(nn.Module):
    def __init__(self, n_rows, n_cols, omega):
        ...
        self.Amn = nn.Parameter(torch.randn(n_rows, n_cols))  # Learnable parameter for Amn
...
for epoch in range(num_epochs):
    optimizer.zero_grad()
    C_fit = model(t)
    loss = criterion(C_fit, C)
    loss.backward()
    optimizer.step()

# Step 4: Verify the results
A_fit = model.Amn.detach()
B_fit = model.Bmn.detach()
...
plt.scatter(t.numpy(), C.numpy()[0][0], label="True Data", alpha=0.5)
plt.plot(t.numpy(), C_fit.detach().numpy()[0][0], label="Fitted Data", color='red')
plt.legend()
plt.xlabel("Time")
plt.ylabel("Value")
